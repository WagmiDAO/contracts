...
contract WagmiAutoStake is Ownable, Pausable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 wagmiAtLastUserAction;
        uint256 lastUserActionTime;
    }

    IERC20 public immutable wagmi;
    IWagmiEarn public immutable wagmiEarn;
    ...
    constructor(
        IERC20 _wagmi,
        IWagmiEarn _wagmiEarn,
        uint256 _stakingPid,
        address _treasury
    ) {
        wagmi = _wagmi;
        wagmiEarn = _wagmiEarn;
    ...
    function deposit(address _user, uint256 _amount) external whenNotPaused nonReentrant {
        ...
        user.wagmiAtLastUserAction = user.shares * balanceOf() / totalShares;
    ...
    function harvest() external whenNotPaused nonReentrant {
        ...
        wagmi.safeTransfer(treasury, currentPerformanceFee);

        uint256 currentCallFee = bal * callFee / 10000;
        wagmi.safeTransfer(msg.sender, currentCallFee);
    ...
    function calculateHarvestWagmiRewards() external view returns (uint256) {
        uint256 amount = IWagmiEarn(wagmiEarn).pendingWagmi(stakingPid, address(this));
    ...
    function calculateTotalPendingWagmiRewards() external view returns (uint256) {
        uint256 amount = IWagmiEarn(wagmiEarn).pendingWagmi(stakingPid, address(this));
        amount = amount + available();
    ...
    function withdraw(uint256 _shares) public nonReentrant {
        ...
        if (bal < currentAmount) {
            ...
            IWagmiEarn(wagmiEarn).withdraw(stakingPid, balWithdraw, true);
        ...
        if (block.timestamp < user.lastDepositedTime + withdrawFeePeriod) {
            ...
            wagmi.safeTransfer(treasury, currentWithdrawFee);
        ...
        if (user.shares > 0) {
            ...
            user.wagmiAtLastUserAction = 0;
        }

        user.lastUserActionTime = block.timestamp;

        wagmi.safeTransfer(msg.sender, currentAmount);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, , ) = IWagmiEarn(wagmiEarn).userInfo(stakingPid, address(this));
        return wagmi.balanceOf(address(this)) + amount;
    }

    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IWagmiEarn(wagmiEarn).deposit(stakingPid, bal, true);
        }
    }
}
